...
export const getCardRulesMeta = (code: string): CardRulesMeta => {
  switch (code) {
    ...
    case 'LOOK_AROUND':
      return {
        targetType: 'Применяется на самого себя',
        defenseInfo: 'Не применяется (действие).',
        obstacleInfo: 'Работает всегда. Не блокируется дверьми.',
        playPhase: 'Фаза действия (Action Phase)',
        tacticalTip: 'Меняет очередность хода и направление обмена картами на противоположное. Позволяет увести ход от опасного соседа!',
      };

    case 'PANIC_FORGETFULNESS':
      return {
        targetType: 'Разыгрывающий игрок (карта Паники)',
        defenseInfo: 'Защита невозможна.',
        obstacleInfo: 'Сбросьте до 3 карт с руки (кроме Нечто и обязательных заражений) и возьмите столько же карт событий.',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Отличный шанс сбросить ненужные карты и обновить руку в поисках полезных действий.',
      };
    ...
    case 'PANIC_BLIND_DATE':
      return {
        targetType: 'Разыгрывающий игрок (карта Паники)',
        defenseInfo: 'Защита невозможна.',
        obstacleInfo: 'Поменяйте 1 карту с руки на верхнюю карту колоды. Ход завершается.',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Слепой обмен с колодой! Ваш ход немедленно завершается.',
      };

    case 'PANIC_CHAIN_REACTION':
      return {
        targetType: 'Все живые игроки за столом (карта Паники)',
        defenseInfo: 'Защита невозможна. Нельзя отказаться от получения карты.',
        obstacleInfo: 'Игнорирует все заколоченные двери и карантины!',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Каждый игрок одновременно отдает 1 карту соседу по направлению хода. Нечто может заразить цель передачей Заражения! Ход заканчивается.',
      };
    ...
    case 'PANIC_OPEN_DOORS':
      return {
        ...
        defenseInfo: 'Защита невозможна.',
        obstacleInfo: 'Все сыгранные карты «Заколоченная дверь» немедленно сбрасываются!',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Все заколоченные проходы снова открыты. Теперь между соседями снова возможны атаки и обмен.',
      };

    case 'PANIC_ONE_TWO_5':
      return {
        targetType: '3-й игрок слева или справа (карта Паники, 5 игроков)',
        defenseInfo: 'Защита невозможна.',
        obstacleInfo: 'Игнорирует заколоченные двери. Если цель в карантине — обмен местами не происходит.',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Вы меняетесь местами с третьим по счёту игроком, перескакивая через стол!',
      };

    case 'PANIC_PARTY_5':
      return {
        targetType: 'Все игроки за столом (карта Паники, 5 игроков)',
        defenseInfo: 'Защита невозможна.',
        obstacleInfo: 'Сбрасывает все двери и карантины. Все игроки попарно меняются местами!',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Полное перемешивание стола! При нечетном числе игроков последний игрок остается на месте.',
      };

    case 'PANIC_GET_AWAY_5':
      return {
        targetType: 'Любой другой игрок за столом не в карантине (карта Паники, 5 игроков)',
        defenseInfo: 'Защита невозможна.',
        obstacleInfo: 'Игнорирует двери. Цель не должна быть в карантине.',
        playPhase: 'Фаза добора карты (Draw Phase)',
        tacticalTip: 'Выбирайте любое место за столом и выгоняйте оттуда соседа!',
      };
